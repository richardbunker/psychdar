...
// Client Assessments
Route::get('/a/{hashed_measure_id}/{hashed_client_id}', [App\Http\Controllers\ClientAssessmentController::class, 'show']);
Route::get('/u/{hashed_user_id}/{hashed_measure_id}/{custom_uri}', function($hashed_user_id, $hashed_measure_id, $custom_uri) {
    $client = App\Models\Client::where('user_id', Hasher::decode($hashed_user_id))->where('custom_client_uri', $custom_uri)->firstOrFail();
    return redirect('/a/'.$hashed_measure_id.'/'.$client->hashed_id);
});
